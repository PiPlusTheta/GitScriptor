...
function all(values3, includeImageAlt, includeHtml) {
  ...
  while (++index4 < values3.length) {
    result[index4] = one2(values3[index4], includeImageAlt, includeHtml);
  }
  return result.join("");
}
function node(value) {
...
function initializeDocument(effects) {
  ...
  function documentContinue(code2) {
    ...
    if (self2.containerState._closeFlow) {
      ...
      ok(point7, "could not find previous flow chunk");
      exitContainers(continued);
      let index4 = indexBeforeExits;
      while (index4 < self2.events.length) {
        self2.events[index4][1].end = { ...point7 };
        index4++;
      }
      splice(
        self2.events,
    ...
    if (events[index4][0] === "enter" && events[index4][1].type === "attentionSequence" && events[index4][1]._close) {
      ...
      while (open--) {
        ...
        context.sliceSerialize(events[open][1]).charCodeAt(0) === context.sliceSerialize(events[index4][1]).charCodeAt(0)) {
          ...
          index4 = open + nextEvents.length - offset - 2;
          break;
        }
      }
    }
...
function createTokenizer(parser, initialize, from) {
  ...
  function now() {
    const { _bufferIndex, _index, line, column, offset } = point7;
    return { _bufferIndex, _index, line, column, offset };
  }
  function defineSkip(value) {
    columnStart[value.line] = value.column;
    accountForPotentialSkip();
    debug("position: define skip: `%j`", point7);
  }
  function main() {
    let chunkIndex;
  ...
  function consume(code2) {
    ...
    if (markdownLineEnding(code2)) {
      point7.line++;
      point7.column = 1;
      point7.offset += code2 === codes.carriageReturnLineFeed ? 2 : 1;
      accountForPotentialSkip();
      debug("position: after eol: `%j`", point7);
  ...
  function store() {
    ...
    function restore() {
      ...
      stack = startStack;
      accountForPotentialSkip();
      debug("position: restore: `%j`", point7);
    }
  }
...
function normalizeUri(value) {
  ...
  while (++index4 < value.length) {
    const code2 = value.charCodeAt(index4);
    let replace = "";
    if (code2 === codes2.percentSign && asciiAlphanumeric2(value.charCodeAt(index4 + 1)) && asciiAlphanumeric2(value.charCodeAt(index4 + 2))) {
      skip = 2;
    } else if (code2 < 128) {
    ...
    if (replace) {
      result.push(value.slice(start2, index4), encodeURIComponent(replace));
